...
```bash
git clone https://github.com/ayalpha/nexora-agent-orchestrator.git
cd nexora-agent-orchestrator
...
1. Push to your GitHub remote (this project ships as [nexora-agent-orchestrator](https://github.com/ayalpha/nexora-agent-orchestrator)).
2. CI workflow is under `.github/workflows/` (project test gate for pushes).
3. Optional Actions secrets under **Settings → Secrets and variables → Actions**:
   - `MESH_API_KEY` — your Mesh key (starts with `rsk_`) if a workflow step needs live Mesh
